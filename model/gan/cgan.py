...
#add two files to investigate embedding matrix
import conditioning_utils_impl
...
def visualize_training_generator(train_step_num, start_time, data_np):
    ...
    plt.axis('off')
    plt.imsave(str(i)+'.png',np.squeeze(data_np), cmap='gray')

def visualize_digits(tensor_to_visualize, filename):
    ...
    plt.imsave(filename, np.squeeze(images_np), cmap='gray')
    return images_np
...
visualize_digits(imgs_to_visualize, filename='samples.png')
...
def conditional_generator_fn(inputs, weight_decay=2.5e-5):
    ...
    with slim.arg_scope(
        ...
        net, embedding_matrix = conditioning_utils_impl.condition_tensor_from_onehot(net, one_hot_labels)
...
conditional_gan_model = gan_model(
...
visualize_digits(cond_generated_data_to_visualize, filename='initial_gen_images.png')
...
with tf.variable_scope(conditional_gan_model.generator_scope, reuse=True):
    eval_images, embedding_matrix = conditional_gan_model.generator_fn(
...
def data(_iter):
    data_file = str(_iter) + '_embedding.txt'
    ...
    matrix.to_csv(str(_iter)+'_orthogonality_matrix.csv')
...
with tf.Session() as sess:
    ...
    with slim.queues.QueueRunners(sess):
        ...
        for i in range(5001):
            ...
            if i == 0:
                embedding = sess.run(embedding_matrix)
                np.savetxt(str(i) + '_embedding.txt', embedding)
                data(i)
            if i % 50 == 0:
                print(i)
