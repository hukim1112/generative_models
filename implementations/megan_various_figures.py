...
from models.gan import megan_64x64 as gan_networks
import argparse
...
def train(checkpoint_path, dataset_path, batch_size, result_path):
	with tf.Graph().as_default():
		...
		visual_feature = {'rotation' : ['left', 'right'], 'transition':['left', 'right']}
		visual_feature_path = '/home/dan/prj/lab/datasets/visual_feature_samples_various_figures'
		...
		with tf.Session() as sess:
			...
			with slim.queues.QueueRunners(sess):
				...
				for i in range(10000):
					...
					if i % 500 == 0:
						visual_gan.varying_categorical_noise(sess, megan_model, 3, unstructured_noise_dims, cont_dim, i, result_path)
						visual_gan.varying_noise_continuous_ndim(sess, megan_model, 3, 0, unstructured_noise_dims, cont_dim
	    																,i, result_path)
						visual_gan.varying_noise_continuous_ndim(sess, megan_model, 3, 1, unstructured_noise_dims, cont_dim
	    																,i, result_path)
